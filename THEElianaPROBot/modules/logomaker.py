...
from THEElianaPROBot import OWNER_ID
from THEElianaPROBot import telethn as tbot
import os 
...
async def lego(event):
 ...
 try:
    ...
    img = Image.open('./THEElianaPROBot/resources/blackbg.jpg')
    draw = ImageDraw.Draw(img)
    image_widthz, image_heightz = img.size
    pointsize = 500
    fillcolor = "gold"
    shadowcolor = "blue"
    font = ImageFont.truetype("./THEElianaPROBot/resources/Chopsic.otf", 330)
...
async def lego(event):
 ...
 try:
    ...
    fillcolor = "white"
    shadowcolor = "blue"
    font = ImageFont.truetype("./THEElianaPROBot/resources/Maghrib.ttf", 1000)
    w, h = draw.textsize(text, font=font)
    h += int(h*0.21)
    image_width, image_height = img.size
    draw.text(((image_widthz-w)/2, (image_heightz-h)/2), text, font=font, fill=(255, 255, 255))
    ...
    img.save(fname2, "png")
    await tbot.send_file(event.chat_id, fname2, caption="Made By @THEElianaPROBot")
    if os.path.exists(fname2):
            os.remove(fname2)
 except Exception as e:
   await event.reply(f'Error Report @AwesomeSupport, {e}')
...
async def logo_gen(event):
    ...
    if event.reply_to_msg_id:
        ...
        fpath_ = "./THEElianaPROBot/resources/fonts/"
    ...
    if not bg_:
        ...
        bg_ = await id_.download_media()
    if not font_:
        fpath_ = glob.glob("THEElianaPROBot/resources/fonts/*")
    ...
    if os.path.exists(font_):
        if not font_.startswith("THEElianaPROBot/resources/fonts"):
            os.remove(font_)
